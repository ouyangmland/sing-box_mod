...
func unwrapUpstreamConn(conn net.Conn) net.Conn {
	for {
		upstream, isUpstreamConn := conn.(upstreamConn)
		if !isUpstreamConn {
			return conn
		}
		upstreamValue := upstream.Upstream()
		upstreamConn, isNetConn := upstreamValue.(net.Conn)
		if !isNetConn || upstreamConn == nil || upstreamConn == conn {
			return conn
		}
		conn = upstreamConn
	}
...
func init() {
	...
	// Reality (uTLS) is wrapped by sing-box as a net.Conn with Upstream(); it is still compatible with Vision.
	tlsRegistry = append(tlsRegistry, func(conn net.Conn) (loaded bool, netConn net.Conn, reflectType reflect.Type, reflectPointer uintptr) {
		conn = unwrapUpstreamConn(conn)

		connWithNetConn, isNetConn := conn.(netConnProvider)
		if !isNetConn {
			return
		}

		connType := reflect.TypeOf(conn)
		if connType == nil || connType.Kind() != reflect.Pointer || connType.Elem().Kind() != reflect.Struct {
			return
		}
		connType = connType.Elem()

		_, hasInput := connType.FieldByName("input")
		_, hasRawInput := connType.FieldByName("rawInput")
		if !hasInput || !hasRawInput {
			return
		}

		return true, connWithNetConn.NetConn(), connType, reflect.ValueOf(conn).Pointer()
	})
}
...
func NewVisionConn(conn net.Conn, tlsConn net.Conn, userUUID [16]byte, logger logger.Logger) (*VisionConn, error) {
	originalTLSConn := tlsConn
	tlsConn = unwrapUpstreamConn(tlsConn)
	...
	if !loaded {
		return nil, E.New("vision: not a valid supported TLS connection: ", reflect.TypeOf(originalTLSConn), " (upstream: ", reflect.TypeOf(tlsConn), ")")
	}
	input, hasInput := reflectType.FieldByName("input")
	rawInput, hasRawInput := reflectType.FieldByName("rawInput")
	if !hasInput || !hasRawInput {
		return nil, E.New("vision: missing required tls fields (input/rawInput) in ", reflectType)
	}
	return &VisionConn{
		Conn:     conn,
